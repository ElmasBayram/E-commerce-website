...
$ürünlistesi = mysqli_fetch_all($result, MYSQLI_ASSOC);
                ...
                <nav class="navbar navbar-expand-lg bg-dark navbar-dark py-3 py-lg-0 px-0">
                    ...
                    <div class="collapse navbar-collapse justify-content-between" id="navbarCollapse">
                        <div class="navbar-nav mr-auto py-0">
                            ...
                            <a href="kirtasiye.php" class="nav-item nav-link active">Kırtasiye</a>
                            <a href="oyuncak.php" class="nav-item nav-link">Oyuncak</a>
                            <a href="kitap.php" class="nav-item nav-link">Kitap</a>
                            <a href="contact.php" class="nav-item nav-link">İletişim</a>
                        </div>
    ...
    <div class="row px-xl-5">
        <div class="col-12">
            <nav class="breadcrumb bg-light mb-30">
                ...
                <a class="breadcrumb-item text-dark" href="kirtasiye.php">Kırtasiye</a>
                <span class="breadcrumb-item active">Ürünler</span>
            </nav>

            <form method="post" action="#">
                <div class="row">
                    <?php foreach($ürünlistesi as $ürün) { ?>
                    <div class="col-md-4">
                        <div class="card mb-4 box-shadow">
                            <img class="card-img-top enlarge-image" style="height: 400px;"
                                src="./admin/resimler/<?php echo $ürün['resim']; ?>" alt="<?php echo $ürün['ad']; ?>">
                            <div class="card-body">
                                <h5 class="card-title"><?php echo $ürün['ad']; ?></h5>
                                <p class="card-text" style="font-size: 20px;"><?php echo $ürün['bilgi']; ?></p>
                                <div class="d-flex justify-content-between align-items-center"></div>
                                <small style="font-size: 20px;"
                                class="text-muted"><?php echo $ürün['fiyat']; ?>₺</small>
                            </div>
                            <div class="btn-group">
                            <button ürün-id="<?php echo $ürün['id']; ?>" role="button" class="button addToCartBtn">Sepete ekle</button>
                            </div>
                        </div>
                    </div>
                    <?php } ?>
...
<script>
  ...
  window.addEventListener('load', function() {
    // Tüm 'addToCartBtn' butonlarını seç
    const addToCartButtons = document.querySelectorAll('.addToCartBtn');

    // Her buton için tıklandığında çalışacak fonksiyonu ekle
    addToCartButtons.forEach(function(button) {
      button.addEventListener('click', function(event) {
        // Formu gönderme varsayılan davranışını engelle
        event.preventDefault();

        // Butondan ürün ID'sini al
        const productId = button.getAttribute('product-id');

        // AJAX isteği göndererek ürünü sepete ekle
        const xhr = new XMLHttpRequest();
        xhr.open('POST', 'sepet_db.php', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        xhr.onreadystatechange = function() {
          if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
            // İşlem tamamlandığında sepetin güncel durumunu göster
           // alert(xhr.responseText);
          }
        };
        xhr.send('ürün_item=' + productId);
      });
    });
  });
</script>
